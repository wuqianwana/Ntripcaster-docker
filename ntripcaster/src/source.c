...
//  char tempbuf[256] = "";
  ...
  while ((con = avl_traverse(info.sources, &trav)) != NULL) {
    ...
    if (con->food.source->audiocast.mount[0] == '/') {
      if (ntripcaster_strcmp(con->food.source->audiocast.mount, mount) == 0) istrue = 1;
    } else {
      if (ntripcaster_strcmp(con->food.source->audiocast.mount, mount+1) == 0) istrue = 1;
    }
...
add_chunk (connection_t *con)
{
  ...
  int source_read_sleep = READ_RETRY_DELAY * 1000; // in microseconds
