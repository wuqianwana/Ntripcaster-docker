...
struct connectionSt;
struct ntrip_request_St;
struct com_requestSt;
typedef int ntripcaster_cmd_function(struct com_requestSt *req);
typedef void ntripcaster_login_function(struct connectionSt *con, struct ntrip_request_St *req);
typedef int ntripcaster_req_function(struct connectionSt *con, struct ntrip_request_St *req);

typedef const char *htf;
typedef avl_tree vartree_t;
typedef htf HttpFunction(char *command, struct connectionSt *clicon, struct connectionSt *sourcecon, const char *ptr, int fd, vartree_t *variables);
typedef htf HttpPrintFunction(void);
typedef int wid_t;
...
typedef struct ntrip_method_St
{
  char *method;
  protocol_t protocol;
  ntripcaster_login_function *login_func;
  ntripcaster_req_function *execute_func;
} ntrip_method_t;
